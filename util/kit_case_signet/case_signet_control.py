...
from util.deal_csv import DealCsv as CSV
...
class GenCase(object):
    ...
    def gen_custom_case_object(self):
        ...
        self.dict_case_custom_spec = get_yaml_data(util.kit_case_signet.dir.INPUT_DIR + 'demo01.yaml')
    ...
    def gen_case_object_type_form(self):
        ...
        for w in self.widget:
            if w["type"] == "text":
                case_widget = self.add_case_object_widget_text(w)
                self.o_csv.add_body(case_widget)

    def add_case_object_widget_text(self, w):
        """添加一个字段的用例二维列表"""
        self.dict_case_form_text = get_yaml_data('default_case_form_text.yaml')
        for (k, v) in w["property"].items():
            self.dict_case_form_text['rule'][k]['value'] = v
            if (k == "min_length"):
                self.dict_case_form_text['rule']['less_length']['value'] = v - 1
            if (k == "max_length"):
                self.dict_case_form_text['rule']['over_length']['value'] = v + 1


        # 根据dict_case_form_text输出字段用例列表
        # 添加行
        case_widget= []
        ...
        step_info = ""
        expect_info = ""
        obj = __import__("propert_gen_case", fromlist=['CaseMap'])
        index = 1
        for(scene, property) in self.dict_case_form_text['rule'].items():
            if hasattr(obj, scene):
                func = getattr(obj, scene)
                scene_info = func(property)
                for a_scene_info in scene_info:
                    ...
                    index = index + 1
        # step_info = '{}'.format(step_info)
        # expect_info = '{}'.format(expect_info)
